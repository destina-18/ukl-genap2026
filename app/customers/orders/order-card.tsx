...
export type OrderItem = {
  ...
  menuImageSnapshot?: string;
  menu_image_snapshot?: string;
...
function getItemImage(item: OrderItem) {
  return (
    ...
    item.cartItem?.menu?.photoUrl ||
    item.cartItem?.menu?.photo_url ||
        ...
        {items.length === 0 ? (
          ...
          <div className="space-y-3">
            {items.map((item, index) => {
              ...
              const menuImage = getItemImage(item);
              const quantity = getItemQuantity(item);
              const price = getItemPrice(item);
                ...
                >
                  <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <div className="flex items-center gap-3">
                      <div className="h-14 w-14 shrink-0 overflow-hidden rounded-xl bg-[#7f1d1d]/10 text-[#7f1d1d]">
                        {menuImage ? (
